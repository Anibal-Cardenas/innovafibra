-- ============================================================================
-- FIX: Unknown column 'descripcion' in 'field list'
-- Date: 2026-01-15
-- Description: Fixes triggers that use 'descripcion' instead of 'observaciones'
-- for the kardex table.
-- ============================================================================

USE sistema_napa;

-- 1. Fix Sales Trigger (trg_after_insert_venta)
...
BEGIN
    ...
    -- Insert into Kardex (using 'observaciones')
    INSERT INTO kardex (
        tipo_movimiento, 
        tipo_item, 
        id_calidad_napa, 
    ...
    VALUES (
        ...
        CONCAT('Venta registrada. Cliente ID: ', NEW.id_cliente)
    );
END$$

DELIMITER ;

-- 2. Fix Production Trigger (trg_produccion_aprobada_inventario)
...
BEGIN
    ...
    IF OLD.estado_validacion != 'aprobado' AND NEW.estado_validacion = 'aprobado' THEN
        ...
        INSERT INTO kardex (tipo_item, tipo_movimiento, cantidad, unidad_medida, referencia_tipo, referencia_id, observaciones) VALUES ('bolsas_plasticas', 'salida', v_peso_bolsas_consumido, 'kg', 'produccion', NEW.id_produccion, CONCAT('Consumo en producción #', NEW.id_produccion));
        ...
        INSERT INTO kardex (tipo_item, id_calidad_napa, tipo_movimiento, cantidad, unidad_medida, referencia_tipo, referencia_id, observaciones) VALUES ('producto_terminado', v_calidad_napa, 'entrada', v_cantidad_producida, 'unidades', 'produccion', NEW.id_produccion, CONCAT('Producción #', NEW.id_produccion, ' aprobada'));
